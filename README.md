...
### For Development
If you want to run the application for development or to explore examples:

```bash
# Clone the repository
git clone https://github.com/rcdexta/react-trello.git
...
This will open Storybook in your browser with all the example boards.

### Using In Your Project
If you want to use react-trello in your own project:
...
// Use the component as described in the Usage section
```

### Troubleshooting

If you encounter yarn.lock syntax errors during installation, try these steps:

```bash
# Remove the corrupted yarn.lock
rm yarn.lock

# Regenerate yarn.lock by running install
yarn install

# If problems persist, try using npm instead
npm install
```

If you encounter webpack errors related to redux-actions with messages like "Module parse failed: Unexpected token (2:12)" or "import.meta.NODE_ENV":

```bash
# Option 1: Make sure you have the correct webpack configuration
# Create or update .storybook/webpack.config.js with a specific rule for redux-actions

# Option 2: Clean installation and use compatible versions
rm -rf node_modules
yarn install

# Option 3: If the error persists, try downgrading redux-actions
yarn add redux-actions@2.6.5 --exact
```

If you see an error about missing `babel-plugin-transform-import-meta`:

```bash
# Install the missing babel plugin
yarn add babel-plugin-transform-import-meta --dev

# Make sure your .babelrc includes this plugin
# Add "babel-plugin-transform-import-meta" to the plugins list
